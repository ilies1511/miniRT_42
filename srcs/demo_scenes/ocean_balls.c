...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/10/09 22:35:59 by iziane            #+#    #+#             */
/*   Updated: 2024/10/09 22:36:03 by iziane           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	nature(t_world *world)
{
	t_light	sun_light;
	t_plane	water;
	t_plane	floor;
	...
	eng_set_transform((t_obj_ptr) & water, mtx_translate(0.00000001, 0.0000001,
			0.000001));
	floor = eng_new_plane();
	floor.base_obj.material = eng_sand();
	eng_set_transform((t_obj_ptr) & floor, mtx_translate(0.0000001, -40.000001,
			0.000001));
	...
	eng_set_transform((t_obj_ptr) & sky, mtx_translate(0.0000001,
			100000000.00001, 0.000001));
	eng_add_obj_to_world(world, (t_obj_ptr) & sky);
}

static void	sph_2(t_world *world)
{
	t_sphere	sph2;
...
static void	init_camera(t_main *m_data)
{
	...
	*(camera) = eng_new_camera(m_data->engine.canvas.width, \
	...
	eng_set_transform((t_obj_ptr)camera, sc_transforme_view(
			new_point(200, 5, -600), new_point(0, 0, 0), new_vec(0, 1, 0)));
}
