...
static void	create_pink_cyl(t_cylinder *cyl, t_world *world, double x, double y,
	double z)
{
	cyl->closed = true;
	...
	eng_set_transform((t_obj_ptr)cyl, mtx_translate(x, y, z));
...
static void	walls(t_world *world)
{
	t_plane		botom;
	t_plane		left_wall;
...
static void	init_world(t_world *world)
{
	...
	while (++i < 6)
	{
		cyl[i] = eng_new_cylinder();
		create_pink_cyl(cyl + i, world, 2 + i, 0, 20);
	}
