...
bool	test_normal_cone(void)
{
	...
	// 	result = false;
	// }
	ist = eng_normal_at((t_obj_ptr) &cone, new_point(1, 1, 1));
	...
	ist = eng_normal_at((t_obj_ptr) &cone, new_point(-1, -1, 0));
...
bool	test_intersec_cone_caps(void)
{
	...
	{
		printf("%s Line %d\n", __FILE__, __LINE__);
		printf("%s Line %d\nTEST1 --> Cone inrersecs count: %lu vs. OG: %d\n", __FILE__, \
	...
	{
		printf("%s Line %d\n", __FILE__, __LINE__);
		printf("%s Line %d\nTEST2 --> Cone inrersecs count: %lu vs. OG: %d\n", __FILE__, \
	...
	{
		printf("%s Line %d\n", __FILE__, __LINE__);
		printf("%s Line %d\nTEST3 --> Cone inrersecs count: %lu vs. OG: %d\n", __FILE__, \
...
bool	test_parallel(void)
{
	...
	if ((!eq_f(intersecs.arr[0].t, 0.35355)))
	{
		...
		result = false;
		printf("%s Line %d\nTEST 0 --> Cone inrersecs count: %lu\n", __FILE__, \
...
bool	test_cone_intersect(void)
{
	...
	if ((intersecs.arr[0].t != 5) || (intersecs.arr[1].t != 5))
	{
		printf("%s Line %d\n", __FILE__, __LINE__);
		printf("TEST 1:\nMY t1: %lf vs. OG: %lf\nMY t2: %lf vs. OG: %lf\n", \
			intersecs.arr[0].t, 5.0, intersecs.arr[1].t, 5.0);
		result = false;
		return (result);
	}
	//TEST2
	...
	if (intersecs2.count == 2)
	{
		...
		return (printf("%s Line %d\nTEST2 --> Cone inrersecs count Im here: %lu\n", __FILE__, \
